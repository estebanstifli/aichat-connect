...
class AIChat_WA_Webhook {
    ...
    public function handle_verify($request){
        ...
        aichat_wa_log_debug('Webhook verify request', [
            'mode' => $mode,
        ...
        if ($mode === 'subscribe' && $token && hash_equals($verify_token, $token)){
            aichat_wa_log_debug('Webhook verify success');
        ...
        aichat_wa_log_debug('Webhook verify failed');
        return new WP_REST_Response('Forbidden', 403);
    }

    public function handle_incoming($request){
        $payload = $request->get_json_params();
        aichat_wa_log_debug('Incoming webhook received', [ 'raw_len' => strlen(file_get_contents('php://input')), 'has_payload' => $payload ? 1:0 ]);
        if (!$payload){
        ...
        if (!empty($payload['entry'][0]['changes'][0]['value']['messages'][0])){
            ...
            if (($msg['type'] ?? '') === 'text'){
                ...
                aichat_wa_log_debug('Processing inbound message', [ 'wa_id'=>$wa_id, 'from'=>$phone, 'business_id'=>$business_id, 'chars'=>strlen($text) ]);
                $service = AIChat_WA_Service::instance();
                $result = $service->handle_incoming_text($phone, $text, $wa_id, $business_id);
                aichat_wa_log_debug('Service processing finished', [ 'wa_id'=>$wa_id, 'has_error'=> isset($result['error'])?1:0 ]);
                return new WP_REST_Response(['processed' => true, 'result' => $result], 200);
            }
        }
        aichat_wa_log_debug('Webhook payload ignored (no text message)');
