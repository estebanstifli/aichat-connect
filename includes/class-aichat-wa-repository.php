...
class AIChat_WA_Repository {
    private static $instance;
    public static function instance(){ if(!self::$instance){ self::$instance = new self(); } return self::$instance; }

    public function get_bot_for_phone($phone){
        global $wpdb; $t = $wpdb->prefix . 'aichat_wa_numbers';
    ...
    public function resolve_bot_slug($business_id, $user_phone){
        ...
        if ($business_id){
            ...
            if ($row) {
                aichat_wa_log_debug('Repo match business id', [ 'business_id'=>$business_id, 'bot_slug'=>$row['bot_slug'], 'service'=>$row['service'] ]);
        ...
        if ($global){
            aichat_wa_log_debug('Repo using global bot fallback', [ 'bot_slug'=>$global ]);
            return ['bot_slug'=>$global,'service'=>'aichat','match_type'=>'global'];
        }
        aichat_wa_log_debug('Repo no mapping found', [ 'business_id'=>$business_id, 'user_phone'=>$user_phone ]);
        return ['bot_slug'=>null,'match_type'=>null];
    }

    public function log_message($data){
        global $wpdb; $t = $wpdb->prefix . 'aichat_wa_messages';
        $wpdb->insert($t, [
        ...
        aichat_wa_log_debug('Repo logged message', [ 'direction'=>$data['direction'] ?? '', 'status'=>$data['status'] ?? '', 'bot_slug'=>$data['bot_slug'] ?? '', 'has_bot_response'=> empty($data['bot_response'])?0:1 ]);
    ...
    public function resolve_credentials($business_id, $user_phone){
        ...
        if ($business_id) {
            ...
            if ($row) {
                aichat_wa_log_debug('Repo credentials business match', [ 'business_id'=>$business_id, 'has_custom_token'=> empty($row['access_token'])?0:1 ]);
                return [
                    'phone_id' => $row['phone'], // El campo phone contiene el phone_number_id de Meta
                    'access_token' => $row['access_token'] ?: get_option('aichat_wa_access_token',''),
                ];
            }
        }
        // Usar configuración global como fallback
        aichat_wa_log_debug('Repo credentials global fallback');
        return [
            'phone_id' => get_option('aichat_wa_default_phone_id',''),
    ...
    public function list_providers($only_active = false){
        global $wpdb; $t = $wpdb->prefix . 'aichat_wa_providers';
